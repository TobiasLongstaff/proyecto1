<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        if($datos != null)
        {
            ...
            $fecha_max = date("Y-m-d",strtotime($fecha_actual."+ 5 days"));

            $cod_producto = $datos->cod_producto;
            $id_pedido = $datos->id_pedido;

            $sql_pro_veri = "SELECT productos.codigo FROM productos INNER JOIN productos_pedidos 
            ON productos_pedidos.cod_producto = productos.cod_stock WHERE productos.codigo = $cod_producto AND productos_pedidos.id_pedido = $id_pedido";
            ...
            {
                $sql_cod_veri = "SELECT productos.codigo FROM productos INNER JOIN cajas ON cajas.codigo = productos.cod_caja 
                ...
                {
                    $sql_car_veri = "SELECT * FROM productos WHERE codigo = '$cod_producto' AND vencimiento > '$fecha_max'";
                    ...
                    {
                        $sql_car_veri = "SELECT codigo FROM productos WHERE codigo = $cod_producto AND cargado = 0";
                        $resultado_car_veri = mysqli_query($conexion, $sql_car_veri);
                        $numero_fila_car_veri = mysqli_num_rows($resultado_car_veri);
                        if($numero_fila_car_veri == '1')
                        {
                            $sql_act_veri = "SELECT productos.cod_stock, stock.descripcion, productos.kilos, productos.vencimiento, productos.fecha, 
                            stock.codigo FROM productos INNER JOIN stock ON stock.codigo = productos.cod_stock 
                            ...
                            {
                                $id_producto = $filas_act_veri['cod_stock'];
                                ...
                                $sql_cant = "SELECT COUNT(codigo) AS cantidad_escaneados FROM productos WHERE activo = '1' AND cod_stock = '$id_producto'";
                    ...
                    else
                    {
                        $json[] = array(
                            'error' => '1',
                            'mensaje' => 'Este producto ya se a punto de vencer y no puede ser utilizado para este pedido',
                        );
                    }
                }
                else
